...
class SyncNovelChapterProgressWithTrackTest {
    ...
    fun `remote ahead marks local chapters to remote`() = runTest {
        ...
        val getNovelChaptersByNovelId = mockk<GetNovelChaptersByNovelId>()
        ...
        coEvery { getNovelChaptersByNovelId.await(novelId) } returns listOf(
        ...
        coEvery { repository.updateAllNovelChapters(capture(chapterUpdatesSlot)) } returns Unit

        val interactor = SyncNovelChapterProgressWithTrack(
            novelChapterRepository = repository,
            insertTrack = insertTrack,
            getNovelChaptersByNovelId = getNovelChaptersByNovelId,
        ...
        coVerify(exactly = 1) { repository.updateAllNovelChapters(any()) }
    ...
    fun `equal progress is no-op`() = runTest {
        ...
        coVerify(exactly = 0) { repository.updateAllNovelChapters(any()) }
        coVerify(exactly = 0) { tracker.update(any(), any()) }
        coVerify(exactly = 0) { insertTrack.await(any()) }
    }
