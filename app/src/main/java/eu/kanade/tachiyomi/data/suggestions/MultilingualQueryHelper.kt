...
/**
 * Helper for multilingual query expansion.
 *
 * Provides:
 * - Genre translation between English and Cyrillic (Russian) for source search
 * - Cyrillic/Latin script detection
 * - Title translation stub (returns null when no translation service is available)
 *
 * In Tadami, this delegates to GoogleTranslationService for live translation.
 * In aniyomi-fork, the translation service is not available, so [translate]
 * returns null and the pipeline relies on the genre map and candidate title
 * resolution alone.
 */
object MultilingualQueryHelper {
    ...
    /**
     * Translate text between Cyrillic and Latin scripts.
     *
     * In Tadami this delegates to GoogleTranslationService. In aniyomi-fork
     * the translation service is not available, so this returns null and the
     * pipeline relies on the genre map and candidate title resolution alone.
     */
    suspend fun translate(text: String): String? {
        if (text.isBlank()) return null
        logcat { "[MultilingualQueryHelper] Translation not available in aniyomi-fork, skipping '$text'" }
        return null
    }
