 ...
 * the series page HTML endpoint. The endpoint is read-only.
 *
...
class NovelUpdatesSimilarSource(
    ...
    private suspend fun searchSeries(query: String): List<NuSeriesStub> {
        ...
        val result = json.decodeFromString<List<NuSeriesDto>>(responseStr)
        return result.map { NuSeriesStub(id = it.id, title = it.title?.rendered ?: "") }
    }
