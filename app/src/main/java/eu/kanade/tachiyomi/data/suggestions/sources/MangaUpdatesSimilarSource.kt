 ...
 * light-novel catalogue (type "Novel").
 */
class MangaUpdatesSimilarSource(
    ...
    override suspend fun fetchSuggestions(seed: SuggestionSeed): List<SuggestionItem> = coroutineScope {
        ...
        val suggestions = try {
            val allSearchResults = mutableListOf<eu.kanade.tachiyomi.data.suggestions.sources.dto.MuSearchResult>()
            ...
            for (candidate in seed.candidateTitles.take(3)) {
                try {
                    ...
                    val searchResponse = json.decodeFromString<MuSearchResponse>(responseStr)
            ...
            val detailResponse = client.newCall(GET(recUrl)).awaitSuccess()
            val detailResponseStr = detailResponse.body?.string() ?: ""
            val detail = json.decodeFromString<MuSeriesDetail>(detailResponseStr)

            val mainRecs = detail.recommendations.sortedByDescending { it.weight }
            val catRecs = detail.categoryRecommendations.sortedByDescending { it.weight }
            ...
            mergedRecs.map { rec ->
                async {
                    try {
                        ...
                        val recDetailResponse = client.newCall(GET(detailUrl)).awaitSuccess()
                        val recDetailStr = recDetailResponse.body?.string() ?: ""
                        val recDetail = json.decodeFromString<MuSeriesDetail>(recDetailStr)
