...
/**
 * Picks the best accent color from a [Palette], preferring vibrant/dominant
 * swatches with good saturation and luminance range.
 *
 * Generalized from the novel-specific [eu.kanade.tachiyomi.util.novel.getBestColor]
 * so manga and anime covers can use the same extraction logic.
 */
fun Palette.getBestColor(): Int? {
    ...
    return when {
        ...
        else -> listOf(vibrantSwatch, lightVibrantSwatch, darkVibrantSwatch).maxByOrNull {
