...
fun SourceFilterNovelDialog(
    ...
    AdaptiveSheet(onDismissRequest = onDismissRequest) {
        LazyColumn {
            ...
            items(filters) {
                NovelFilterItem(it, updateFilters)
            }
        }
    }
}
...
@Composable
private fun NovelFilterItem(filter: NovelFilter<*>, onUpdate: () -> Unit) {
    when (filter) {
        is NovelFilter.Header -> {
            HeadingItem(filter.name)
        }
        is NovelFilter.Separator -> {
            HorizontalDivider()
        }
            ...
            CollapsibleBox(
                ...
                Column {
                    filter.state
                        ...
                        .map { NovelFilterItem(filter = it, onUpdate = onUpdate) }
                }
            }
        }
    }
...
private fun Int.toTriStateFilter(): TriState {
    return when (this) {
        NovelFilter.TriState.STATE_IGNORE -> TriState.DISABLED
        NovelFilter.TriState.STATE_INCLUDE -> TriState.ENABLED_IS
        NovelFilter.TriState.STATE_EXCLUDE -> TriState.ENABLED_NOT
...
private fun TriState.toTriStateInt(): Int {
    return when (this) {
        TriState.DISABLED -> NovelFilter.TriState.STATE_IGNORE
        TriState.ENABLED_IS -> NovelFilter.TriState.STATE_INCLUDE
        TriState.ENABLED_NOT -> NovelFilter.TriState.STATE_EXCLUDE
    }
}
