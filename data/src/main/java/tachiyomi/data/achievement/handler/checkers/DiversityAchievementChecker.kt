...
class DiversityAchievementChecker(
    ...
    suspend fun getSourceDiversity(): Int {
        ...
        val mangaSources = mangaRepository.getLibraryManga().map { it.manga.source }

        val animeSources = animeRepository.getLibraryAnime().map { it.anime.source }

        val novelSources = novelRepository.getLibraryNovels().map { it.novel.source }
